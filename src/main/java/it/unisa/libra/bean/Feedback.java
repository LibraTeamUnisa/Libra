...
@NamedQueries({@NamedQuery(name = "Feedback.findAll", query = "SELECT f FROM Feedback f"),
    @NamedQuery(name = "Feedback.findByType",
        query = "SELECT f FROM Feedback f WHERE f.progettoFormativo.id = :idProgettoFormativo AND f.domanda.tipo= :tipoDomanda ORDER BY f.domanda.id")})
