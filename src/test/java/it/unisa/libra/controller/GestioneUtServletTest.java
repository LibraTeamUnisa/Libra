...
public class GestioneUtServletTest {
  ...
  public void shouldCreateUser() throws Exception { // Dovrebbe creare un utente
    ...
    Utente ut = (Utente) em
        .createQuery("SELECT u FROM Utente u WHERE u.email = '" + utente.getEmail() + "'")
        .getSingleResult();
  ...
  public void shouldCreatePresident() throws Exception { // Dovrebbe creare un presidente
    ...
    Utente ut =
        (Utente) em.createQuery("SELECT u FROM Utente u WHERE u.email = " + utente.getEmail())
            .getSingleResult();
    assertEquals("lucaizzo@gmail.com", utente.getEmail());

    // Cerca il presidente all'interno del DB

    Presidente pr = (Presidente) em
        .createQuery("SELECT p FROM Presidente p WHERE p.cognome = " + pres.getCognome())
        .getSingleResult();
  ...
  public void shouldCreateTutorEsterno() throws Exception { // Dovrebbe creare un TutorEsterno
    ...
    TutorEsterno tutu = (TutorEsterno) em
        .createQuery("SELECT t FROM TutorEsterno t WHERE t.cognome = " + tut.getCognome())
        .getSingleResult();
