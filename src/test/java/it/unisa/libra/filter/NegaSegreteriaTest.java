...
public class NegaSegreteriaTest {

  NegaSegreteria filter = new NegaSegreteria();
  ...
  public void nonAutorizzatoTest() throws Exception {
    when(request.getSession().getAttribute("utenteRuolo")).thenReturn("Segreteria");
    when(request.getContextPath()).thenReturn("provaLibra");
    filter.doFilter(request, response, chain);
    verify(response).sendRedirect("provaLibra" + JspPagesIndex.ACCESSO_NEGATO);
  }
  ...
  public void paginaConcessaTest() throws Exception {
    when(request.getSession().getAttribute("utenteRuolo")).thenReturn("NotSegreteriaRuolo");
    filter.doFilter(request, response, chain);
