 ...
 * registrato: quest'ultimo ricever� la password smarrita tramite una email
...
public class RecuperoPasswordServlet extends HttpServlet {
		    ...
		    throws ServletException, IOException
  {
	  ...
	  try 
	  {
        ...
        response.getWriter().write("L'email � stata inviata all'indirizzo specificato.");
        response.getWriter().flush();
    } else {
        response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
        response.getWriter().write("L'email inserita non � valida.");
        response.getWriter().flush();
    }
    } catch(Exception ex) {
